...
local function isDeathOverlayActive()
    return GameOverActive and IndieCrossGameOverStyle == 'Cuphead'
end
...
function onUpdate(el)
    -- Death overlay menu logic MUST run BEFORE the early return
    -- to handle menu animations, selection input, and button positioning
    if isDeathOverlayActive() then
        if IndieCrossGameOverStyle == 'Cuphead' then
            ...
            if GameOverActive then
                ...
                for eventNotes = 0, getProperty('eventNotes.length')-1 do
                    setPropertyFromGroup('eventNotes', eventNotes, 'active', false)
                end
            end
...
function onStepHit()
    ...
    elseif curStep == 1174 then
        ...
        callScript('data/knockout/customevents','shotHit',{'MugMan'})
    end
end
function onBeatHit()
    if isDeathOverlayActive() then
