...
local function hideRoundabout(roundTag)
    if roundTag == nil or string.len(roundTag) < 1 then
        return
    end
    if not stringStartsWith(roundTag, 'Roundabout') then
        roundTag = 'Roundabout'..roundTag
    end
    if luaSpriteExists(roundTag) then
        setProperty(roundTag..'.visible', false)
        setProperty(roundTag..'.active', false)
        setProperty(roundTag..'.alpha', 0)
        setProperty(roundTag..'.velocity.x', 0)
        setProperty(roundTag..'.velocity.y', 0)
    end
end

local function getRoundaboutTagFromTimer(tag)
    local roundTag = string.gsub(tag, 'dodgeDoubleAttack', '')
    if string.len(roundTag) < 1 then
        return nil
    end
    if not stringStartsWith(roundTag, 'Roundabout') then
        roundTag = 'Roundabout'..roundTag
...
function onUpdate(el)
    ...
    for i, time in pairs(cupAttacks) do
        if time - getSongPosition() < (doAlert and 600 or 700) then
            playAnim('dad', '')
            callScript('data/knockout/script','dodgeT')
            if doAlert then
                callScript('data/knockout/script','createAlert')
...
function attackCup()
    ...
    callScript('data/knockout/script','unfillCard')
end
...
function onTimerCompleted(tag)
    if isGameplayFrozen() and tag ~= 'GameOverCuphead' then
        return
    end
    ...
    if string.find(tag,'dodgeDoubleAttack',0,true) then
        local roundTag = getRoundaboutTagFromTimer(tag)
        setProperty('isCameraOnForcedPos',false)
        if dodge == 1 then
            bfDodge()
            if roundTag ~= nil and stringStartsWith(tag, 'dodgeDoubleAttackRoundabout') then
                runTimer('hideRoundabout'..roundTag, 2)
            end
        elseif dodge == 2 then
            bfHurt()
            if roundTag ~= nil and luaSpriteExists(roundTag) then
                ...
                setBlendMode(burst, 'add')
                setObjectCamera(burst, 'other')
    ...
    if stringStartsWith(tag,'dodgeCupAttack') then
        if dodge == 1 then
            bfDodge()
        elseif dodge == 2 then
            bfHurt()
    ...
    if stringStartsWith(tag, 'hideRoundaboutRoundabout') then
        hideRoundabout(string.gsub(tag, 'hideRoundabout', ''))
        return
    end
    if tag == 'dodgeRound' then
        if dodge == 1 then
            bfDodge()
		elseif dodge == 2 then
            bfHurt()
