...
class ShapeDebug extends Debug{
    ...
    public override function drawPolygon(polygon:Dynamic,colour:Int){
        ...
        if(zpp_inner.xnull){
            {
                if(#if flash untyped __is__(polygon,Array)#else (polygon is Array)#end){
                    ...
                    for(vite in lv){
                        ...
                        #end
                        #if(!NAPE_RELEASE_BUILD)
                        if(!#if flash untyped __is__(vite,Vec2)#else (vite is Vec2)#end)throw "Error: Array<Vec2> contains non Vec2 objects";
                        #end
                        var p:Vec2=vite;
                        {
                            #if(!NAPE_RELEASE_BUILD)
                ...
                else if(#if flash untyped __is__(polygon,Vec2List)#else (polygon is Vec2List)#end){
                ...
                else if(#if flash untyped __is__(polygon,GeomPoly)#else (polygon is GeomPoly)#end){
    ...
    public override function draw(object:Dynamic){
        {
            ...
            #end
            if(zpp_inner.xnull){
                if(#if flash untyped __is__(object,Space)#else (object is Space)#end)zpp_inner_zn.draw_space(cast(object,Space).zpp_inner,null,1.0,true);
                else if(#if flash untyped __is__(object,Compound)#else (object is Compound)#end)zpp_inner_zn.draw_compound(cast(object,Compound).zpp_inner,null,1.0,true);
                else if(#if flash untyped __is__(object,Body)#else (object is Body)#end)zpp_inner_zn.draw_body(cast(object,Body).zpp_inner,null,1.0,true);
                else if(#if flash untyped __is__(object,Shape)#else (object is Shape)#end)zpp_inner_zn.draw_shape(cast(object,Shape).zpp_inner,null,1.0,true);
                else if(#if flash untyped __is__(object,Constraint)#else (object is Constraint)#end)cast(object,Constraint).zpp_inner.draw(this);
                else{
                    #if(!NAPE_RELEASE_BUILD)
                ...
                if(#if flash untyped __is__(object,Space)#else (object is Space)#end)zpp_inner_zn.draw_space(cast(object,Space).zpp_inner,zpp_inner.xform,zpp_inner.xdet,false);
                else if(#if flash untyped __is__(object,Body)#else (object is Body)#end)zpp_inner_zn.draw_body(cast(object,Body).zpp_inner,zpp_inner.xform,zpp_inner.xdet,false);
                else if(#if flash untyped __is__(object,Shape)#else (object is Shape)#end)zpp_inner_zn.draw_shape(cast(object,Shape).zpp_inner,zpp_inner.xform,zpp_inner.xdet,false);
                else if(#if flash untyped __is__(object,Constraint)#else (object is Constraint)#end)cast(object,Constraint).zpp_inner.draw(this);
                else{
                    #if(!NAPE_RELEASE_BUILD)
                    throw "Error: Unhandled object type for Debug draw";
