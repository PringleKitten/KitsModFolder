...
class ZPP_OptionType{
    ...
    public function append(list:ZNPList_ZPP_CbType,val:Dynamic){
        ...
        #end
        if(#if flash untyped __is__(val,CbType)#else (val is CbType)#end){
            var cb:CbType=val;
            append_type(list,cb.zpp_inner);
        }
        else if(#if flash untyped __is__(val,CbTypeList)#else (val is CbTypeList)#end){
            var cbs:CbTypeList=val;
            for(cb in cbs)append_type(list,cb.zpp_inner);
        }
        else if(#if flash10#if flash untyped __is__(val,ZPP_Const.cbtypevector)#else (val is ZPP_Const.cbtypevector)#end
        #else false #end){
            #if flash10 var cbs:flash.Vector<CbType>=val;
            for(cb in cbs)append_type(list,cb.zpp_inner);
            #end
        }
        else if(#if flash untyped __is__(val,Array)#else (val is Array)#end){
            var cbs:Array<Dynamic>=val;
            for(cb in cbs){
                #if(!NAPE_RELEASE_BUILD)
                if(!#if flash untyped __is__(cb,CbType)#else (cb is CbType)#end){
    ...
    public static function argument(val:Dynamic):OptionType{
        ...
        else if(#if flash untyped __is__(val,OptionType)#else (val is OptionType)#end)val;
