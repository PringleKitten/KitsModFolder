...
class PlayState extends MusicBeatState
{
	...
	{
		#if desktop
		if (health > 0 && !paused) DiscordClient.changePresence(detailsPausedText, Highscore.floorDecimal(ratingPercent * 100, 2)+"%" + "-Miss: " + songMisses + "-Hit: " + songHits + "-Combo: " + combo, iconP2.getCharacter());
		#end
		...
		switch(eventName) {
			...
			case 'Add Camera Zoom':
				if(ClientPrefs.data.camZooms && FlxG.camera.zoom < 1.35) {
					...
					if(ClientPrefs.data.camZoomsHud && FlxG.camera.zoom < 1.45)
						{
							var hudZoom:Float = Std.parseFloat(value2);
							if(Math.isNaN(hudZoom)) hudZoom = 0.03;
						}
						else {
							if(ClientPrefs.data.camZoomsBg && FlxG.camera.zoom < 1.45)
						{
	...
	override function sectionHit()
	{
		if (SONG.notes[curSection] != null)
		{
			...
			if (camZooming && FlxG.camera.zoom < 1.8 && ClientPrefs.data.camZoomsHud)
				{
					camHUD.zoom += 0.06 * camZoomingMult;
				}
			if (camZooming && FlxG.camera.zoom < 1.8 && ClientPrefs.data.camZoomsBg)
				{
